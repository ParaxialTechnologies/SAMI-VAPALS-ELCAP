...
if [[ -f /home/osehra/etc/env.conf ]]
then
    source /home/osehra/etc/env.conf
else
    echo "Missing environment configuration file [/home/osehra/etc/env.conf]" >&2
    exit 2
fi
...
then
    ...
    $gtm_dist/mumps -run stop^%webreq
...
mprocesses=$(lsof -t /home/osehra/data/globals/*.dat)
if [[ -z $mprocesses ]]
then
