:doctitle: SAMI Patching Checklist for 18-17-pho-b1 

[role="h1 center"]
SAMI Patching Checklist for 18-17-pho-b1

-------------------------------------------------------------------------------
 0. Project discussion
  0.1. Which changes will be included
	   Updated routine SAMICSV will be installed on the Phoenix VAPALS-iELCAP
	   system as step one of a three step fix for the "data-dump/crash" issue

  0.2. Timeline: Release goal 1/28/2025, Actual release

 1. Project set up
X   1.1. Prepare checklist
X   1.2. prepare recipe doc
/   1.3. prepare template.txt doc
x   1.4. create new directory for patch (i.e. dist/18-17-pho-1/b1)
N   1.5. create new directories 'routines' and within that 'old' and 'new'
N   1.6. copy pre-development routines into routines/old
X   1.7. git pull/push
...
X   1.9. list log routines
   ...
   1.10. write change descriptions for template.txt
   1.11. update dashboard (Google Drive)
...
N    1.12. Regrettable Bodge script
  		run within docs/www
  		../regrettable-bodge.py
  		then commit

N 2. Update routines
   2.0. update recipe with before checksums -- do recipe^ARJTRW
...
N   3.1. load in vapalsyotta ~/lib/silver/a-sami-vapals-elcap--vv-paraxtech-github
		...
		~/run/routines/
N  3.2. copy routines to ~/run/routines directory
  		cp -v ROUTINE.m ~/run/routines/
N   3.3. run XINDEX (refer to XINDEX below)
N   3.4. fix any bugs,
N   3.4.1.update first four lines of routines (See VERSION CONTROL LINES FORMAT below)
...
X   3.6. update recipe with after information
 ...
 4. KIDS Build 
   4.0 copy all new routines to ~/run/routines just in case, also to routines/after
        in current dist.
   4.1 In linux cd ~/run/out/sami/ then mkdir 18-17-pho-1 if necessary, cd 18-21
       mkdir b1
   4.2. Re-enter mumps
   ...
   4.4. export to out dock (Transport distribution) 
		HF: /home/osehra/run/out/sami/18-17-pho-1/b1/sami-18-17-pho-b1.kid
   ...
   4.5. copy to personal repo, typing following command when you are in the correct
        ...
        scp 'vapalsyotta:/home/osehra/run/out/sami/18-17-pho-1/b1/*.kid' ./
N    4.6. Add before checksums to .kid file
    4.7. push to sami repo, pull to vapalsyotta in 
       ~/lib/silver/a-sami-vapals-elcap--vv-paraxtech-github/dist/
 ...
 or from 18-21-b1.txt
  7.0 Pull to vapalsyotta and move KIDS to ~/run/in/sami/18-17-pho-1/b1/
  7.1. install on vapalsyotta (& revise if necessary),
  HF: /home/osehra/run/in/sami/18-17-pho-1/b1/sami-18-17-pho-1-b1.kid
