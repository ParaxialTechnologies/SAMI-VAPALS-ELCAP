...
:vista-longversion: Version 18.0, Patch ID #17, Seq #17, Test 3
:nmsp: SAMI
:lnmsp: sami
:appname: VAPALS-ELCAP
:va-version: {nmsp}*18.0*17 SEQ #17 T3
:priority: High
...
** <<toc,Contents>>
* <<desc,Description>>
** <<issue-21-218,Login: Implement single sign-on>>
** <<"issue-21-221","Logout: Upon sign-out, redirect to the correct web page">>
...
{nmsp} {version} adds one high-priority update to add _name_ and _last5_ to a
`.csv` report.
...
**Unnumbered Issue (high priority):** A report is available to be run by the
system administrator that outputs all patient data for statistical
analysis. Lines did not include patient{nbsp}ID.

**Solution:** Add `saminame` (full patient name) and `last5` (First letter of
last name and last four digits of{nbsp}SSN) in each line of report.
