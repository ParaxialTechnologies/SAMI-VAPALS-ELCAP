:doctitle: SAMI Patching Checklist for 18-20-b1 

[role="h1 center"]
SAMI Patching Checklist for 18-20-b1

-------------------------------------------------------------------------------
 0. Project discussion
  0.1. Which changes will be included
	    Update CSV reports to include MRN, studyID
	    Interrum fix of Data Dump bug
	    Change Image Upload to match on MRN, not name
	    
  0.2. Timeline: Release goal , Actual release 2024-11-11

 1. Project set up
X  1.1. create new directory for patch (i.e. dist/18-20/b1)
X  1.2. create new directories 'routines' and within that 'old' and 'new'
X  1.3. copy pre-development routines into routines/old
X  1.4. git pull
X  1.5. prepare checklist
X  1.6. prepare recipe doc
X  1.7. prepare template.txt doc  
X  1.8. git pull/push
X  1.9. list updated routines
		SAMICSV
		SAMIDCM1
		SAMIDCM2
		SAMIPAT
  
X  1.10. list log routines
        none

X  1.11. write change descriptions for template.txt  
X  1.12. update dashboard (Google Drive)
     ...
     (sami pax) use pax^ARJTXRW	to determine dependencies
        7   11   12   14   15   16
	   17   18   19
X  1.13. Regrettable Bodge script not needed this patch
  		run within docs/www
  		../regrettable-bodge.py
  		then commit

 2. Update routines
X  2.0. update recipe with before checksums -- do recipe^ARJTXRW 
...
X  3.7. update dashboard https://docs.google.com/spreadsheets/d/1tWV8QQfLURjmypkk0Wbddwad_8UbUv1f9sf6ext1_Q0/edit?gid=1956251173#gid=1956251173
      see temp dash below (sami vers) use dash^ARJTXRW for dates and checksums
X  3.8. git pull/push, pull to vapalsyotta

 4. KIDS Build 
X  4.0 copy all new routines to ~/run/routines just in case
X  4.1 In linux cd ~/run/out/sami/ then mkdir 18-20 if necessary, cd 18-20
       mkdir b1
...
X   5.3. Review index.adoc (start on line 101)
X   5.4. Make {version}.adoc and {version}.tx doc from template.txt and recipe
       ...
       dtool --makewebzip
X   5.5. Review {version}.adoc and {version}.tx doc and make any needed updates
X   5.6. git push/pull
...
X   6.3. update version website's link
 ...
 7. Test Install
X   7.1 Copy old routines to ~/run/routines
X   7.2 Follow instructions on website (https://vistaexpertise.net/project/screening-plus/dist/18-20/b1/) 
 or from 18-20-b1.txt
X  7.3 Pull to vapalsyotta and move KIDS to ~/run/in/sami/18-20/b1/ or follow directions
  	on website.
X  7.4. install on vapalsyotta (& revise if necessary),
  HF: /home/osehra/run/in/sami/18-20/b1/sami-18-20-b1.kid
X  7.5. test changes on vapalsyotta (vapalsyotta.vistaexpertise.net:9080/vapals)
...
X  7.7  copy new routines to repo routines 
  7.8. send any necessary updates for user and tech manual changes
  7.9. Skype team and ask them to test
...
temporary dash, do dash^ARJTXRW to get dates for routines, then do select^ARJTXRW(.routines), 
enter routines, then run this snippet	
s r="" f  s r=$o(routines(r)) q:r=""  write !,r,?10,$$sumf^%r(r),?20,$$sumb^%r(r),?35,$$sume^%r(r)
...
Add back when cache is available
