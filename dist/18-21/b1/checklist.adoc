...
-------------------------------------------------------------------------------
 0. Project discussion
  0.1. Which changes will be included
	    ...
	    Fix problem of second record being created due to misspelled name upon
	    	CT Upload

  0.2. Timeline: Release goal , Actual release

 1. Project set up
   1.1. create new directory for patch (i.e. dist/18-20/b1)
   1.2. create new directories 'routines' and within that 'old' and 'new'
   1.3. copy pre-development routines into routines/old
   1.4. git pull
   1.5. prepare checklist
   1.6. prepare recipe doc
   1.7. prepare template.txt doc  
   1.8. git pull/push
   1.9. list updated routines
		SAMICSV
		SAMIDCM1
		SAMIDCM2
		SAMIPAT
  
   1.10. list log routines
        none

   1.11. write change descriptions for template.txt  
   1.12. update dashboard (Google Drive)
     ...
     (sami pax) use pax^ARJTXRW	to determine dependencies
        7   11   12   14   15   16
	   17   18   19
   1.13. Regrettable Bodge script not needed this patch
  		run within docs/www
