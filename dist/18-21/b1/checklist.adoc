:doctitle: SAMI Patching Checklist for 18-21-b1 

[role="h1 center"]
SAMI Patching Checklist for 18-21-b1

-------------------------------------------------------------------------------
 0. Project discussion
  0.1. Which changes will be included
	    Support url override for viewing images 
	    Implement FILECSV, which generates spreadsheets from the linus files
	    that store the forms at Mt. Sinai and inserts a column called 
	    "separator". All fields left of the separator are in our data 
	    dictionaries, all fields to the right of the separator are not in the
	    DDs but are in the form files at MSH.
	    Add the X-ray form: cxform.
	    Make other form changes

  0.2. Timeline: Release goal , Actual release

 1. Project set up
X   1.1. Prepare checklist
X   1.2. prepare recipe doc
X   1.3. prepare template.txt doc
		Note: For next patch, I'd like to change it so that the YottaDB/GT.M 
		stuff goes before the Cache stuff. At this point, more people are 
		using yotta than Cache.
X   1.4. create new directory for patch (i.e. dist/18-20/b1)
X   1.5. create new directories 'routines' and within that 'old' and 'new'
X   1.6. copy pre-development routines into routines/old
X   1.7. git pull/push
X   1.8. list updated routines
		SAMICAS2
		SAMICAS3
		SAMICSV
		SAMICSV2
...
X   1.9. list log routines
        SAMICUL
        SAMIHUL
        SAMIURUL

x   1.10. write change descriptions for template.txt
/   1.10.b Add instructions to template to add cxform (see Adding form to SPlus)
X   1.11. update dashboard (Google Drive)
     ...
     don't use SAMIPAT in list
X    1.12. Regrettable Bodge script
  		run within docs/www
  		../regrettable-bodge.py
  		then commit

 2. Update routines
X   2.0. update recipe with before checksums -- do recipe^ARJTRW
X   2.1. Update SAMIPAT for pre/post-install if needed
      do SETPARM^SAMIPARM("SYS","samiSystemVersion","sami-18-21-b1")
   2.2. update routine histories, convert to new short version ids, update dates & times in each routine
...
/   3.1. load in vapalsyotta ~/lib/silver/a-sami-vapals-elcap--vv-paraxtech-github
		 ./compare2.sh in routines before copying routines to make sure no 
...
X   3.4. fix any bugs,
X   3.4.1.update first four lines of routines (See VERSION CONTROL LINES FORMAT below)
   3.4.2 if you wish, use ^%ZPT1 to compare first through third lines of routine for 
...
X   3.6. update recipe with after information
  3.7. update dashboard https://docs.google.com/spreadsheets/d/1tWV8QQfLURjmypkk0Wbddwad_8UbUv1f9sf6ext1_Q0/edit?gid=1956251173#gid=1956251173
      (sami vers) use dash^ARJTRW for dates and checksums
   3.8. git pull/push, pull to vapalsyotta

 4. KIDS Build 
X   4.0 copy all new routines to ~/run/routines just in case, also to routines/new
        in current dist.
X   4.1 In linux cd ~/run/out/sami/ then mkdir 18-21 if necessary, cd 18-21
       mkdir b1
...
X   4.4. export to out dock (Transport distribution) 
		HF: /home/osehra/run/out/sami/18-21/b1/sami-18-21-b1.kid
		Header: Released SAMI*18.0*21 SEQ #21
X   4.5. copy to personal repo, typing following command when you are in the correct
        dist directory on your laptop: 
        scp 'vapalsyotta:/home/osehra/run/out/sami/18-21/b1/*.kid' ./
X    4.6. Add before checksums to .kid file
X    4.7. push to sami repo, pull to vapalsyotta in 
       ~/lib/silver/a-sami-vapals-elcap--vv-paraxtech-github/dist/18-20/b1$

 5. Finish draft documents  
    5.1. Review recipe and make any needed changes
    5.2. Review template.txt and make any needed changes
    5.3. Review index.adoc (start on line 101)
    5.4. Make {version}.adoc and {version}.tx doc from template.txt and recipe
       dtool --makescript   builds upgrade
       dtool --textfile in the build directory
       dtool --makewebzip
    5.5. Review {version}.adoc and {version}.tx doc and make any needed updates
    5.6. git push/pull
 ...
 7. Test Install
  Copy old routines to ~/run/routines
  Follow instructions on website (https://vistaexpertise.net/project/screening-plus/dist/18-19/b1/) 
 or from 18-21-b1.txt
  7.0 Pull to vapalsyotta and move KIDS to ~/run/in/sami/18-18/b1/
  7.1. install on vapalsyotta (& revise if necessary),
  HF: /home/osehra/run/in/sami/18-21/b1/sami-18-21-b1.kid
...
8. Release Patch
  ...
  8.4. Update dashboard (sami envs) (sami pax Date)
  8.5. Copy new routines to lib/silver/a-sami-(etc)/routines file+
...
Do select^ARJTXRW(.routines) puts the routine names entered into the symbol table like 
this example:routines("SAMICAS3”)=“”.  You can then use this snippet of code 
s r="" f  s r=$o(routines(r)) q:r=""  write !,r,?10,$$sumb^%r(r),?25,$$sume^%r(r),?35,$$sumf^%r(r)  
to determine the B, E, and F checksums for the routines entered into select^ARJTXRW.
...
KIDS BUILD
	...
	PAGE 1/5
	Name should be entered, enter distribution date (format MM/DD/YYYY), tab through
	description and Environment Routine.
...
Enter a Host File: /home/osehra/run/out/sami/18-21/b1/sami-18-21-b1.kid
Header Comment: Released SAMI*18.0*21 SEQ #21
