:patchtitle: Add cxform, generate spreadsheets from current MSH files, etc.
...
-------------------------------------------------------------------------------
$TXT by YAW,LINDA linda.yaw@vistaexpertise.net (KIDS) 2025-01-24t01:41z
-------------------------------------------------------------------------------
...
| Depends on  | SAMI 18-7 (††SAMI*18.0*7 SEQ #7††) +
                ...
                link:../../18-20/[SAMI 18-20] (††SAMI*18.0*20 SEQ #20††)
|==============================================================================
...
{nmsp} {version} adds four updates to add new features or fix bugs.
...
* Support URL override for viewing images
  (link:#issue-a[24-0000], high{nbsp}priority)
* Generate spreadsheets from current MSH files
  (link:#issue-b[24-0000], high{nbsp}priority)
* Add the X-ray form: ††cxform††
  (link:#issue-c[24-0000], high{nbsp}priority)
* Make form changes for MSH
  (link:#issue-d[24-0000], high{nbsp}priority)

[[issue-a]]
=== Support URL override for viewing images

**Request 24-0000 (high priority):** Changes were made to the DICOM server
which necessitated the change.
   
**Solution:** Feature was added to override the URL.

**Reported:** Karthik (date unknown).
   
[[issue-b]]
=== Generate spreadsheets from current MSH files
 
**Request 24-0000 (high priority):** Implement FILECSV, which generates
spreadsheets from the linus files that store the forms at Mt.{nbsp}Sinai.
   
**Solution:** Inserts a column called "separator". All fields left of the
separator are in our data dictionaries, all fields to the right of the
separator are not in the DDs but are in the form files at MSH.

**Reported:** Artit Jirapatnakul (date unknown).

[[issue-c]]
=== Add the X-ray form: ‘cxform’
 
**Request 24-0000 (high priority):** Ethiopia does a lot of their
scanning by x-ray instead of CT, so they need a form to properly log
the information observed in the x-rays.

**Solution:** Develop cxform and integrate information into database.

**Reported:** Rick Avila (date unknown).

[[issue-d]]
=== Make form changes for MSH
 
**Request 24-0000 (high priority):** Reports ran incorrectly.

**Solution:** The changes made to the intake form for MSH eliminated
three variables and changed the meaning of another variable so that
the report generator could not tell who was enrolled or active. The
logic in the report generator was enhanced to work with either the VA
or MSH versions of the intake form.
...
YottaDB, GT.M, or Caché session (depending on your particular installation),
sign in to Vista, and then call the direct-mode interface `^%RO` (“routine
out”).
...
`mumps` command on YottaDB/GT.M or the `csession` command on Caché:
...
In the text below, ⮐ indicates pressing the Enter/Return key.
...
[ttsp]##Select Kernel Installation & Distribution System \... Option## prompt
select +Installation+:
...
your screen. Here's a sample session.
...
|==============================================================================
|           | Dates                 | By
| Developed | 2024-11-04/2025-01-12 | George P. Lilly (VEN) +
                                      Dominic DiNatale (PAR) +
                                      Frederick D. S. Marshall (VEN) +
                                      Kenneth W. McGlothlen (VEN) +
                                      Linda M. R. Yaw (VEN)
| Tested    | 2024-11-06/2025-01-12 | George P. Lilly (VEN) +
                                      Artit Jirapatnakul (VA-PHO) +
                                      Linda M. R. Yaw (VEN)
| Released  | 2025-01-23            | Frederick D. S. Marshall (VEN) +
                                      Kenneth W. McGlothlen (VEN) +
                                      Linda M. R. Yaw (VEN)
|==============================================================================
