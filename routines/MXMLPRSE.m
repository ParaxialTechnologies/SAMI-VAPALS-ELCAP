MXMLPRSE ;SAIC/DKM - XML Parser ; 6/17/18 7:12pm
...
EN(DOC,CBK,OPTION) ;
 ...
 K ^TMP(ID,$J)
 ; VEN/lgc modified to fix complex subscripts
POO D FIXSS
 Q
 ; Open a document
...
NEXT(X,Y) Q $$NEXT^MXMLPRS0(.X,.Y)
 ;
 ; VEN/lgc Fix complex subscripts in XWBP variables
 ;  specifically for setting TIU text
 ; e.g.
 ;  XWBDATA("PARAMS")=",.XWBP1,.XWBP2,.XWBP3"
 ;  XWBDATA("PARAMS",1)="XWBP1"
 ;  XWBDATA("PARAMS",2)="XWBP2"
 ;  XWBDATA("PARAMS",3)="XWBP3"
 ;  XWBP1=23
 ;  XWBP2("HDR")="1^1"
 ;  XWBP2("{TEXT},1,0")="Current cigarette smoker:"
 ;  XWBP2("{TEXT},2,0")="    # of years: 15"
 ;  XWBP3=0
FIXSS N NODE,SNODE,XWBPSS,SSNODE,ORIGSS,NEWSS,VALUE
 S NODE=$NA(XWBDATA("PARAMS")),SNODE=$P(NODE,")")
 F  S NODE=$Q(@NODE) Q:NODE'[SNODE  D
 . S XWBPSS=@NODE
 . S SSNODE=$NA(@XWBPSS)
 . F  S SSNODE=$Q(@SSNODE) Q:SSNODE=""  D
 .. S ORIGSS=$P($P(SSNODE,"(",2),")")
 .. Q:ORIGSS'["{"
 .. S ORIGSS=$TR($TR(ORIGSS,""""),"{")
 .. S VALUE=@SSNODE
 ..; OK. Now separate out each subscript (e.g. {TEXT,1,and 0)
 ..;  and turn into string if it starts with {
 ..; S SS1="TEXT",SS2=1,SS3=0, then S XWBP2(SS1,SS2,SS3)=""
 ..;   so kill the original node [SSNODE] and reset with value
 .. N SS F I=1:1:$L(ORIGSS,",") S SS(I)=$P(ORIGSS,",",I)
 .. I ($L(ORIGSS,",")=2) S @XWBPSS@(SS(1),SS(2))=VALUE Q
 .. I ($L(ORIGSS,",")=3) S @XWBPSS@(SS(1),SS(2),SS(3))=VALUE Q
 .. I ($L(ORIGSS,",")=4) S @XWBPSS@(SS(1),SS(2),SS(3),SS(4))=VALUE
 ;
 Q
